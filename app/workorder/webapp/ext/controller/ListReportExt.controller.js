sap.ui.define(
  ...
  function (
    ...
    return ControllerExtension.extend(
      "maintenance.workorder.workorder.ext.controller.ListReportExt",
      {
        override: {
          onInit: function () {
            this._setRoleModel();
        ...
        onOpenActionsDialog: async function (event) {
          ...
          const latestWorkOrder = await this._getLatestWorkOrder(
        ...
        _setRoleModel: async function () {
          ...
          try {
            const userInfo = await this._getJson(
          ...
          if (normalizedStatus === "assigned") {
            actionButton = new Button({
              ...
              press: async function () {
                await this._executeWorkOrderAction(
                  dialog,
                  workOrder,
                  "startWork",
                );
        ...
        _executeWorkOrderAction: async function (dialog, workOrder, action) {
          ...
          this._logActionStatus(
            startWork ? "Start Work" : "Complete Task",
            current,
          );
        ...
        _getProcedure: async function (equipmentId, maintenanceType) {
          ...
          const filters = [
            `EquipmentID eq '${this._odataString(equipmentId)}'`,
          ];

          if (maintenanceType) {
            filters.push(
              `MaintenanceCategory eq '${this._odataString(maintenanceType)}'`,
            );
          }
            ...
            items: [
              ...
              new Text({
                text:
                  ...
                  "No procedure found for this equipment and maintenance type.",
                wrapping: true,
              }),
            ],
          }).addStyleClass("sapUiSmallMargin");
        ...
        _refreshNow: function () {
          const extensionAPI =
            this.base.getExtensionAPI && this.base.getExtensionAPI();
        ...
        _hideActionsColumn: function () {
          ...
          table.initialized().then(() => {
            const columns = table.getColumns();

            columns.forEach(function (column) {
              const header = column.getHeader();

              if (header && header.getText && header.getText() === "Actions") {
